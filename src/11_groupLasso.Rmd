...
foreach(i = 1:length(unique_drugs), .combine='rbind') %dopar% {
  d = unique_drugs[i]
  
  regression_results <- tibble(penalty=character(), alpha=double(), drug=character(), train=logical(), lambda=double(), mse = double(), corr=double())
  ...
  for (alph in alphas){
    mod <- oem(x = X_train, y = new_y_train,
             family = "gaussian",
             penalty = "grp.lasso.net",# "sparse.grp.lasso", "lasso", ""),
             lambda=lambdas,
             alpha=alph,
             groups=grps,
             standardize = FALSE,
